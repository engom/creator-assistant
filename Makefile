...
sync-env:                          ## Push .env secrets to SSM + hot-reload API container on EC2
	...
	@aws ssm send-command \
	    ...
	    --parameters 'commands=["set -e","cd /opt/pubiq","for param in API_KEYS TIKTOK_CLIENT_ID TIKTOK_CLIENT_SECRET TIKTOK_REDIRECT_URI JWT_SECRET POSTGRES_USER POSTGRES_PASSWORD POSTGRES_DB CORS_ORIGINS LLM_MODEL AWS_REGION; do val=$$(aws ssm get-parameter --region eu-west-3 --name \"/pubiq/$$param\" --with-decryption --query Parameter.Value --output text 2>/dev/null) || continue; grep -q \"^$$param=\" docker/.env && sed -i \"s|^$$param=.*|$$param=$$val|\" docker/.env || echo \"$$param=$$val\" >> docker/.env; done","PG_USER=$$(grep ^POSTGRES_USER docker/.env | cut -d= -f2)","PG_PASS=$$(grep ^POSTGRES_PASSWORD docker/.env | cut -d= -f2)","PG_DB=$$(grep ^POSTGRES_DB docker/.env | cut -d= -f2)","DB_URL=\"postgresql://$${PG_USER}:$${PG_PASS}@postgres:5432/$${PG_DB}\"","grep -q \"^DATABASE_URL=\" docker/.env && sed -i \"s|^DATABASE_URL=.*|DATABASE_URL=$${DB_URL}|\" docker/.env || echo \"DATABASE_URL=$${DB_URL}\" >> docker/.env","grep -q \"^DBOS_SYSTEM_DATABASE_URL=\" docker/.env && sed -i \"s|^DBOS_SYSTEM_DATABASE_URL=.*|DBOS_SYSTEM_DATABASE_URL=$${DB_URL}|\" docker/.env || echo \"DBOS_SYSTEM_DATABASE_URL=$${DB_URL}\" >> docker/.env","until docker compose -p pubiq -f docker/docker-compose.yml exec -T postgres pg_isready -U $${PG_USER} > /dev/null 2>&1; do sleep 1; done","docker compose -p pubiq -f docker/docker-compose.yml up -d --force-recreate --no-build api","echo done"]' \
